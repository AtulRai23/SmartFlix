...
u = st.text_input(
    "Enter User Id", key="user_input")
m = st.text_input(
    "Enter MovieId", key="movie_input")
...
if submit:
    with st.spinner("Processing..."):
        ...
        train_data = Dataset.load_from_df(
            reg_train[['user', 'movie', 'rating']], reader)
        trainset = train_data.build_full_trainset()
        testset = list(zip(reg_test_df.user.values,
                       reg_test_df.movie.values, reg_test_df.rating.values))

        # Initialize the model
        svd = SVD(n_factors=100, biased=True, random_state=30, verbose=False)
        svd_train_results, svd_test_results = run_surprise(
            svd, trainset, testset, verbose=False)

        # Initialize the model
        svdpp = SVDpp(n_factors=50, random_state=30, verbose=False)
        svdpp_train_results, svdpp_test_results = run_surprise(
            svdpp, trainset, testset, verbose=False)
        ...
        predicted_ratings = [svd.predict(
            user_id, movie_id).est for movie_id in all_movie_ids]
        ...
        sorted_movie_ratings = sorted(
            movie_ratings, key=lambda x: x[1], reverse=True)
