...
def get_gemini_similarity(movie1, movie2):
    ...
    response = model.generate_content(
        f"Roughly calculate similarities between {movie1} and {movie2}.")
...
if submit:
    with st.spinner("Processing..."):
        ...
        if mv_id in movie_titles.index:
            st.write(f"Movie id {mv_id} corresponds to:",
                     movie_titles.loc[mv_id].values[1])
            ...
            for index, row in top_10_movies.iterrows():
                similarity_score = get_gemini_similarity(
                    movie_titles.loc[mv_id].values[1], row['title'])
                st.write(f"Similarity score between {movie_titles.loc[mv_id].values[1]} and {
                         row['title']}: {similarity_score}")
        else:
